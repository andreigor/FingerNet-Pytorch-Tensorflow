...
parser.add_argument('input_fingerprint_image_folder', type=str, default="../datasets/NISTSD27/images/",
                    help='input fingerprint image folder path')
args = parser.parse_args()

import tensorflow as tf

os.environ["CUDA_VISIBLE_DEVICES"]=args.GPU
config = tf.compat.v1.ConfigProto(gpu_options=tf.compat.v1.GPUOptions(allow_growth=True), log_device_placement=True)
...
deploy_set = [args.input_fingerprint_image_folder]
...
def get_main_net(input_shape=(512,512,1), weights_path=None):
    ...
    toc = time()
    print('Pre processamento: ', toc - tic)
    ...
    conv=MaxPooling2D(pool_size=(2,2),strides=(2,2))(conv)
    tic = time()
    print('Extrator de caracteristicas: ', tic - toc)
...
def deploy(deploy_set, set_name=None):
    ...
    for i in range(0,len(img_name)):
        ...
        print('Tempo da rede: ', time() - start)
        time_afterconv = time()
